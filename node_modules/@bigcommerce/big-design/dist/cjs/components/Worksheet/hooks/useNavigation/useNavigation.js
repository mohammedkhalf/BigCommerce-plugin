...
const useNavigation = selectedCell => {
  const {
    store,
    useStore
  ...
  const setSelectedRows = useStore(store, (0, _shallow.useShallow)(state => state.setSelectedRows));
  const isValidPosition = (0, _react.useCallback)(position => {
  ...
  const isHidden = (0, _react.useCallback)(rowIndex => {
  ...
  const getNextOffset = (0, _react.useCallback)(({
    columnIndex,
    rowIndex
  }) => {
    if (rowIndex === 0) {
  ...
  const navigate = (0, _react.useCallback)(offset => {
    if (!selectedCell) {
      return;
    }
    const newPosition = {
