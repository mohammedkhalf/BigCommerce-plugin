"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.useSelectable = void 0;
var _react = require("react");
var _utils = require("../../../utils");
// prettier-ignore
const getDefaultSelectedValues = ({
  nodes,
  selectedNodes,
  type
}) => {
  if (type === 'radio') {
    const selectedNode = (0, _utils.depthFirstSearch)(nodes, ({
      id,
      value
    }) => selectedNodes.includes(id) && value !== undefined);
    if (selectedNode?.value !== undefined) {
      return [selectedNode.value];
    }
  }
  if (type === 'multi') {
    return (0, _utils.depthFirstSearch)(nodes, ({
      id,
      value
    }) => selectedNodes.includes(id) && value !== undefined, false)?.reduce((acc, node) => node.value !== undefined ? [...acc, node.value] : acc, []) ?? [];
  }
  return [];
};

// prettier-ignore
const useSelectable = ({
  defaultSelected,
  disabledNodes,
  nodes,
  onSelectionChange,
  type
}) => {
  const [selectedNodes, setSelectedNodes] = (0, _react.useState)(defaultSelected ?? []);
  const [selectedValues, setSelectedValues] = (0, _react.useState)(() => type ? getDefaultSelectedValues({
    nodes,
    selectedNodes,
    type
  }) : []);
  (0, _react.useEffect)(() => {
    if (defaultSelected) {
      setSelectedNodes(defaultSelected);
      setSelectedValues(getDefaultSelectedValues({
        nodes,
        selectedNodes: defaultSelected,
        type
      }));
    }
  }, [defaultSelected, nodes, type]);
  (0, _react.useEffect)(() => {
    if (type === 'radio') {
      if (selectedNodes.length >= 1) {
        setSelectedNodes(prevSelected => prevSelected.slice(0, 1));
        setSelectedValues(prevValues => prevValues.slice(0, 1));
      } else if (nodes.length) {
        const firstSelectedNode = (0, _utils.depthFirstSearch)(nodes, node => node.value !== undefined, true);

        // Need to check for undefined value since TS can't determine from DFS check.
        if (firstSelectedNode?.value !== undefined) {
          setSelectedNodes([firstSelectedNode.id]);
          setSelectedValues([firstSelectedNode.value]);
        }
      }
    }
  }, [nodes, selectedNodes.length, setSelectedNodes, type]);
  (0, _react.useEffect)(() => {
    if (typeof onSelectionChange === 'function') {
      onSelectionChange(selectedValues);
    }
  }, [onSelectionChange, selectedValues]);
  const onSelect = (0, _react.useCallback)((nodeId, value) => {
    if (disabledNodes?.includes(nodeId)) {
      return;
    }
    if (type === 'multi') {
      if (selectedNodes.includes(nodeId)) {
        setSelectedNodes(prevNodes => prevNodes.filter(prevNodeId => prevNodeId !== nodeId));
        setSelectedValues(prevValues => prevValues.filter(prevValue => prevValue !== value));
      } else {
        setSelectedNodes([...selectedNodes, nodeId]);
        setSelectedValues([...selectedValues, value]);
      }
    }
    if (type === 'radio' && !selectedNodes.includes(nodeId)) {
      setSelectedNodes([nodeId]);
      setSelectedValues([value]);
    }
  }, [disabledNodes, selectedNodes, selectedValues, type]);
  return {
    selectedNodes,
    onSelect
  };
};
exports.useSelectable = useSelectable;