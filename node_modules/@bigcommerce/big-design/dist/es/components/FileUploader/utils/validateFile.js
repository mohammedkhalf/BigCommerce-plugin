export const validateFileFormat = (file, fileFormats = '') => {
  ...
  return allowedFormats.some(format => {
    ...
    } else if (format.startsWith('audio/') || format.startsWith('video/') || format.startsWith('image/')) {
...
export const validateFiles = (files, validators) => files.flatMap((file, idx) => validators.filter(({
  validator
}) => !validator(file)).map(({
  message,
  type
