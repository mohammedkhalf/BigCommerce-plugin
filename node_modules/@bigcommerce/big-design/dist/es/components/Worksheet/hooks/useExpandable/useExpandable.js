...
export const useExpandable = rowId => {
  const {
    store,
    useStore
  } = useWorksheetStore();
  ...
  const setHiddenRows = useStore(store, useShallow(state => state.setHiddenRows));
  ...
  const onToggle = useCallback(isExpanded => {
    if (isExpanded) {
      onCollapse();
    } else {
      onExpand();
