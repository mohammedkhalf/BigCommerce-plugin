...
// prettier-ignore
const nodeExistsInTree = (nodes, id) => nodes.some(node => node.id === id || (node.children ? nodeExistsInTree(node.children, id) : false));
const createObserveElementRect = fallbackHeightRef => (instance, cb) => {
...
export const useTreeVirtualizer = ({
  ...
  const nodeIndexMap = useMemo(() => new Map(flatNodes.map(({
    node
  }, index) => [node.id, index])), [flatNodes]);
  const focusedIndex = useMemo(() => enabled ? nodeIndexMap.get(focusedNode) ?? -1 : -1, [enabled, nodeIndexMap, focusedNode]);
  ...
  useEffect(() => {
    ...
    virtualizer.scrollToIndex(focusedIndexRef.current, {
      behavior: 'auto'
    });
