import React, { memo } from 'react';
import { toTransientMarginProps } from '../../helpers/margins/margins';
import { StyledH0, StyledH1, StyledH2, StyledH3, StyledH4, StyledHR, StyledSmall, StyledText } from './styled';
import { jsx as _jsx } from "react/jsx-runtime";
function toTransientTextProps(props) {
  return {
    $ellipsis: props.ellipsis,
    $bold: props.bold,
    $capitalize: props.capitalize,
    $italic: props.italic,
    $lowercase: props.lowercase,
    $strikethrough: props.strikethrough,
    $underline: props.underline,
    $uppercase: props.uppercase
  };
}
const validHeadingTags = new Set(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']);

// Private
export const StyleableText = StyledText;
export const StyleableSmall = StyledSmall;
export const StyleableH0 = StyledH0;
export const StyleableH1 = StyledH1;
export const StyleableH2 = StyledH2;
export const StyleableH3 = StyledH3;
export const StyleableH4 = StyledH4;
export const StyleableHR = StyledHR;

// Public
export const Text = /*#__PURE__*/memo(props => {
  const {
    className,
    style,
    margin,
    marginTop,
    marginRight,
    marginBottom,
    marginLeft,
    marginVertical,
    marginHorizontal,
    ellipsis,
    bold,
    capitalize,
    italic,
    lowercase,
    strikethrough,
    underline,
    uppercase,
    ...domProps
  } = props;
  return /*#__PURE__*/_jsx(StyleableText, {
    ...domProps,
    ...toTransientMarginProps(props),
    ...toTransientTextProps(props)
  });
});
export const Small = /*#__PURE__*/memo(props => {
  const {
    className,
    style,
    margin,
    marginTop,
    marginRight,
    marginBottom,
    marginLeft,
    marginVertical,
    marginHorizontal,
    ellipsis,
    bold,
    capitalize,
    italic,
    lowercase,
    strikethrough,
    underline,
    uppercase,
    ...domProps
  } = props;
  return /*#__PURE__*/_jsx(StyleableSmall, {
    ...domProps,
    ...toTransientMarginProps(props),
    ...toTransientTextProps(props)
  });
});
export const HR = /*#__PURE__*/memo(props => {
  const {
    className,
    style,
    margin,
    marginTop,
    marginRight,
    marginBottom,
    marginLeft,
    marginVertical,
    marginHorizontal,
    ...domProps
  } = props;
  return /*#__PURE__*/_jsx(StyleableHR, {
    ...domProps,
    ...toTransientMarginProps(props)
  });
});
export const H0 = /*#__PURE__*/memo(props => {
  const {
    className,
    style,
    as,
    margin,
    marginTop,
    marginRight,
    marginBottom,
    marginLeft,
    marginVertical,
    marginHorizontal,
    ellipsis,
    ...domProps
  } = props;
  return /*#__PURE__*/_jsx(StyleableH0, {
    as: getHeadingTag('h1', as),
    ...domProps,
    ...toTransientMarginProps(props),
    ...toTransientTextProps(props)
  });
});
export const H1 = /*#__PURE__*/memo(props => {
  const {
    className,
    style,
    as,
    margin,
    marginTop,
    marginRight,
    marginBottom,
    marginLeft,
    marginVertical,
    marginHorizontal,
    ellipsis,
    ...domProps
  } = props;
  return /*#__PURE__*/_jsx(StyleableH1, {
    as: getHeadingTag('h1', as),
    ...domProps,
    ...toTransientMarginProps(props),
    ...toTransientTextProps(props)
  });
});
export const H2 = /*#__PURE__*/memo(props => {
  const {
    className,
    style,
    as,
    margin,
    marginTop,
    marginRight,
    marginBottom,
    marginLeft,
    marginVertical,
    marginHorizontal,
    ellipsis,
    ...domProps
  } = props;
  return /*#__PURE__*/_jsx(StyleableH2, {
    as: getHeadingTag('h2', as),
    ...domProps,
    ...toTransientMarginProps(props),
    ...toTransientTextProps(props)
  });
});
export const H3 = /*#__PURE__*/memo(props => {
  const {
    className,
    style,
    as,
    margin,
    marginTop,
    marginRight,
    marginBottom,
    marginLeft,
    marginVertical,
    marginHorizontal,
    ellipsis,
    ...domProps
  } = props;
  return /*#__PURE__*/_jsx(StyleableH3, {
    as: getHeadingTag('h3', as),
    ...domProps,
    ...toTransientMarginProps(props),
    ...toTransientTextProps(props)
  });
});
export const H4 = /*#__PURE__*/memo(props => {
  const {
    className,
    style,
    as,
    margin,
    marginTop,
    marginRight,
    marginBottom,
    marginLeft,
    marginVertical,
    marginHorizontal,
    ellipsis,
    ...domProps
  } = props;
  return /*#__PURE__*/_jsx(StyleableH4, {
    as: getHeadingTag('h4', as),
    ...domProps,
    ...toTransientMarginProps(props),
    ...toTransientTextProps(props)
  });
});
const getHeadingTag = (defaultTag, tag) => {
  return tag && validHeadingTags.has(tag) ? tag : defaultTag;
};
Text.displayName = 'Text';
Small.displayName = 'Small';
HR.displayName = 'HR';
H0.displayName = 'H0';
H1.displayName = 'H1';
H2.displayName = 'H2';
H3.displayName = 'H3';
H4.displayName = 'H4';