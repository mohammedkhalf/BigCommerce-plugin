...
final class TamaraOrderStatus
{
    ...
    public static function fromTamaraStatus(string $status): ?PaymentSessionStatus
    {
        ...
        return match (true) {
            ...
            in_array($status, ['fully_refunded', 'fully refunded'], true) => PaymentSessionStatus::Refunded,
    ...
    public static function isFullyRefunded(array $payload): bool
    {
        $status = (string) (
        ...
        if (self::fromTamaraStatus($status) === PaymentSessionStatus::Refunded) {
            return true;
        }

        $refundedAmount = data_get($payload, 'data.refunded_amount.amount')
            ?? data_get($payload, 'refunded_amount.amount');
        ...
        $totalAmount = data_get($payload, 'data.total_amount.amount')
            ...
            ?? data_get($payload, 'data.captured_amount.amount')
            ?? data_get($payload, 'captured_amount.amount');

        return $totalAmount !== null
            ? (float) $refundedAmount >= (float) $totalAmount
            : true;
    }
